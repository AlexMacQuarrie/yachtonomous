...
from logging import boat_logger
from tools import Angle

'''
TODO
- May want to ensure the loop always takes T seconds (actual time must be < T)
- Sensor reading just return a const, and inputs do nothing
'''
...
def run() -> None:
    ...
    if np.abs(x_hat_init[3]) > boat_config.crit_wind_angle:
        x_hat_init[2] = np.pi/4.0
    elif x_hat_init[3] < 0:
        x_hat_init[2] = 3.0*np.pi/8.0
    else:
        x_hat_init[2] = np.pi/8.0
    ...
    # Log initial results
    logger = boat_logger(test_config.log_en)
    logger.log_results(x_hat, x_d, u, u_act)
    ...
    for k in range(1, x_d.shape[1]):
        ...
        logger.log_results(x_hat, x_d[:, k], u, u_act)
